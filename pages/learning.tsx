...
function Learning() {
  const flip = (
    course:
      | { name: string; rating: string; description: string }
      | { name: string; rating: number; description: string }
  ) => {
    ...
    if (element.innerHTML === course.description) {
      element.innerHTML =
        "<pre>" + course.rating + "\n" + course.name + "</pre>";
    } else element.innerHTML = course.description;
  };
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col mx-10 font-sans">
        ...
        <h4 className="heading_2">
          My main studies are done at the University of Cape Town with
          supplements online (Hint: tap the squares!)
        </h4>
        ...
        <h4 className="heading_2">
          I tend to believe education goes far beyond the classroom, and this is
          my attempt to validate that hypothesis
        </h4>
        <div className="box-container">
